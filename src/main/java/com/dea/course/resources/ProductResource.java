...
import com.dea.course.services.ProductService;
...
@RequestMapping(value = "/products") //Essa anotatio dá um nome ao recurso. Como é uma classe de Usuário que está sendo usado numa entidade Product(ProductResource), coloco o nome de /users.
public class ProductResource {
	...
	private ProductService service;//dependencia para o ServiceProduct. Para ela funcioanar, precisa estar registrada como componente do spring. Para isso preciso ir na classe ProductService e registra-la como componente. Fazemos isso usando uma anotation no começo da classe ProductService como @Service.
	
	@GetMapping //Para indicar que esse método responde a requisição do tipo get do http eu coloco essa anotation.
	public ResponseEntity<List<Product>> findAll() { //Tipo de retorno desse método é o ResponseEntity, que é um tipo especifico do String Boot para retornar resposta de requisições web. Esse ResponseEntity é genérico. Ele espera um tipo e o tipo de resposta vai ser Product, da classe Product.
		List<Product> list = service.findAll(); //findAll, método que vai retornar os meus usuários.
