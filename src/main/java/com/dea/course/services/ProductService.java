...
public class ProductService {
	...
	private ProductRepository repository; //declaro a dependencia para o ProductRepository
	
	public List<Product> findAll() { //método para buscar todos os usuários. Lista do tipo Product e nome do método de findAll. Fizemos uma operação na camada de serviço onde ela repassa a chamada para o repository.findAll(), abaixo.
		return repository.findAll(); //retorna o repository com a operação findAll do método.
	}//Lá no meu ProductResource que é o controlador Rest, preciso atualizar a implementação do findAll(), declarando uma lista de Usuário e chamando o método findAll().
	
	public Product findById(Long id) { //operação para buscar os usuários por Id 
		Optional<Product> obj = repository.findById(id); //chamo o método repository.findById retornando o Optinona<Product>
		return obj.get(); //A operação get() do optional vai retornar o objeto do tipo <Product> que estiver dentro do optional
	}
